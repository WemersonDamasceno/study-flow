...
void main() {
  test('SaveTokenState props', () {
    const state1 = SaveTokenState();
    const state2 = SaveTokenState();

    expect(state1, state2);
    expect(state1.hashCode, state2.hashCode);
  });

  test('SaveTokenState copyWith', () {
    const state = SaveTokenState(status: StatusEnum.success);
  ...
  test('SaveTokenState copyWith no changes', () {
    const state = SaveTokenState(status: StatusEnum.success);
  ...
  test('SaveTokenState copyWith with changes', () {
    const state = SaveTokenState(status: StatusEnum.success);
